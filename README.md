# AegisAi: Advanced SAST & DAST Powered by LLM Agents

**AegisAi** is an innovative security tool suite that leverages the power of Large Language Models (LLMs) to enhance Static and Dynamic Application Security Testing (SAST & DAST). It aims to provide developers and security professionals with intelligent vulnerability analysis, insightful reporting, and actionable remediation guidance, all within a streamlined and user-friendly interface.

## Features

AegisAi offers a range of security analysis capabilities, including:
...
* **Interactive Security Debugging:**
  * **AegisAi Fortify:** An AI-powered security debugger for interactive vulnerability analysis and remediation guidance.
...
Follow these steps to get AegisAi up and running on your local machine.

### Prerequisites

Before you begin, ensure you have the following installed:

1. **Docker:** AegisAi utilizes Docker and Docker Compose for easy setup and dependency management. Install [Docker Desktop](https://www.docker.com/products/docker-desktop/) for your operating system.

2. **OWASP ZAP:** AegisAi Sword (DAST component) relies on OWASP ZAP. Download and install [OWASP ZAP](https://www.zaproxy.org/). Ensure ZAP is running and accessible, typically on `localhost:8080`.
...
```bash
git clone https://github.com/DeeptanshNagar/AegisAi.git
cd AegisAi
```
...
* Start the AegisAi application components: `aegisai-shield.py`, `aegisai-sword.py`, and `Fortify.py`.
...
Once the `start.sh` script completes successfully, AegisAi will be running and accessible in your web browser at the following URLs:

* **AegisAi Shield (SAST Agents):** `http://localhost:8501`
* **AegisAi Sword (DAST Scanner):** `http://localhost:8502`
* **AegisAi Fortify (Interactive Debugger):** `http://localhost:8503`

## Usage

AegisAi is composed of three main tools, each accessible through a separate Streamlit application:

* **AegisAi Shield 🛡️:** This is the main SAST interface. Navigate to `http://localhost:8501` to:
...
* **AegisAi Sword ⚔️:** This tool provides DAST capabilities. Access it at `http://localhost:8502` to:
...
* **AegisAi Fortify 🛡️:** The interactive security debugger is available at `http://localhost:8503`.
...
AegisAi is built with a modular architecture, comprising the following key components:

* **Streamlit Applications:**
  * `aegisai-shield.py`: Provides the user interface for SAST agents and analysis reports.
  * `aegisai-sword.py`: Offers DAST scanning functionality using OWASP ZAP and LLM reporting.
...
* **Core Functions:** Utility functions for code scraping, file handling, Git operations, and LLM interaction (defined in `aegisai-shield.py` and `aegisai-sword.py`).
...
Thank you for using AegisAi! We hope it helps you build more secure applications.
